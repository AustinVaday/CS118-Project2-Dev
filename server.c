...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    // If client sends ack (after syn-ack)
    ...
    else 
    {
      ...
      if (is_ack_bit_set(&header_rec)) 
      {
          ...
          // if (window[0].acked == 0)
          // {
          //   send_packets_from_ack = 0;
          // }
          // else
          // {
          //   send_packets_from_ack = 1;
          // }
      ...
      // Set ack num and seq num accordingly, since here is when 
      // seq_num = seq_num + strlen(dataBuf);
      // ack_num = ack_num;

      for (int i = 0; i < numPacketsToSend; i++)
      {
        seq_num += responseBufSizes[i] + HEADERSIZE;
    ...
    for (int i = 0; i < numPacketsToSend; i++)
    {
      ...
      if (isProcessingACKs)
      {
          window[WINDOWSIZE / PACKETSIZE - 1].tcpObject = tcpObject;
          window[WINDOWSIZE / PACKETSIZE - 1].tcpObjectLength = tcpObjectLength;
      ...
      // MIGHT NOT NEED THE BELOW FLAG!?!?
      if (!do_not_send_packet)
      {
